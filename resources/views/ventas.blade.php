<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <meta http-equiv="X-UA-Compatible" content="ie=edge" />
  <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet" />

  <title>Punto de Venta</title>

  <!-- Font Awesome -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css" />

  <!--Material Design-->

  <script src="https://unpkg.com/material-components-web@latest/dist/material-components-web.min.js"></script>

  <link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons" />
  <!--Materialize-->
  <!-- Compiled and minified CSS -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css" />


    <!-- Styles -->
    <link href="{{ asset('css/modales.css') }}" rel="stylesheet">
    <link href="{{ asset('css/estilos.css') }}" rel="stylesheet">
    <link href="{{ asset('css/responsive.css') }}" rel="stylesheet">
    <link href="{{ asset('css/configuracion.css') }}" rel="stylesheet">


</head>

<body>
  <div class="contenedor">
    <header class="header" style="padding-top: 0;padding-left:0px;padding-right: 0; 
        box-shadow: 1px 1px #999999;">


      <!-- Dropdown Structure -->
      <ul id="dropdown1" class="dropdown-content">
        <li><a href="#!">Perfil</a></li>
        <li><a class="modal-trigger" href="#cerrarelturno">Cerrar turno</a></li>
        <li class="divider"></li>
        <li>
        
            <a class="dropdown-item" >
            Cerrar sesión
         </a>

        
        </li>
      </ul>
      <nav style="padding: 10px;
            box-shadow: none;
            background-color: transparent;">
        <div class="nav-wrapper">
          <a href="#!" class="brand-logo center"
            style="display:flex;text-align: center;justify-content: center;align-items: center;"><svg width="419"
              height="80" viewBox="0 0 419 80" fill="none" xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink">
              <rect width="419" height="80" fill="url(#pattern0)" />
              <defs>
                <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                  <use xlink:href="#image0" transform="scale(0.00214592 0.011236)" />
                </pattern>
                <image id="image0" width="419" height="80"
                  xlink:href="data:image/png;base64," />
              </defs>
            </svg></a>
          <a href="#" data-target="mobile-demo" style="display: block;" class="sidenav-trigger">

            <i class="material-icons grey-text" style="font-size: 30px;">menu</i></a>
          <ul class="right">

            <!-- Dropdown Trigger -->
            <li style="color: grey;">
              <a class="dropdown-trigger" href="#!" data-target="dropdown1"
                style="display: flex;justify-content: space-around;color:gray">
                <i style="font-size: 20px; margin-right: 0px;" class="material-icons grey--text">account_circle</i>
         
                <i class="material-icons">arrow_drop_down</i></a></li>
          </ul>
        </div>
      </nav>

      <ul class="sidenav nav-menu" id="mobile-demo">
        <li>
          <a class="sidenav-close" style="width:32%;margin-bottom:10px" id="cerrarnavegacion"><i
              class="material-icons">keyboard_backspace</i>
          </a>
        </li>
        <li>
          <a href="ventas" class="">Ventas<i class="material-icons">check</i>
          </a>
        </li>
        <li>
          <a href="clientes" class="">Clientes<i class="material-icons">people_outline</i></a>
        </li>
        <li>
          <a href="productos" class="">Productos<i class="material-icons">local_offer</i></a>
        </li>
        <li>
          <a href="inventario" class="">Inventario<i class="material-icons">assignment_turned_in</i></a>
        </li>
        <li>
          <a href="cortedeldia" class="">Corte<i class="material-icons">book_online</i></a>
        </li>
        <li>
          <a href="mantenimiento" class="">Mantenimiento<i class="material-icons">build</i></a>
        </li>
        <li>
          <a href="configuracion" class="">Configuración<i class="material-icons">miscellaneous_services</i></a>
        </li>

      </ul>

      <!-- cerrarelturno Modal Structure -->
      <div id="cerrarelturno" class="modal">

        <h5><strong>¿Seguro que desea cerrar el turno?</strong></h5>

        <div style="display: flex;justify-content: space-evenly;align-items: center;">
          <a id="" class="aceptar">SI</a>
          <a id="" class="modal-close cancelar">NO</a>

        </div>

      </div>
    </header>




    <main class="contenido">
      <!--menu Principal-->
      <div class="menu-principal">

        <!-- Fila 1  input / Buscar / Producto sin código (F1) -->
        <div class="one">
  

          <input  name="inputbuscar" id="inputBuscar" type="search" placeholder='... buscar'>

  
        </div>

        <!-- Producto s/códito Trigger -->
        <div class="three">
          <a class="ancla modal-trigger" href="#sincodigodinero">
            <i class="material-icons">label_off</i>
            Producto Sín Código(F1)
          </a>
          <!--  Producto s/códito Modal Structure -->
          <div id="sincodigodinero" class="modal">

            <h6>
              <i class="material-icons orange-text">shopping_cart
              </i>Producto Sín Código
            </h6>

            <div class="contenedormodal">
              <!-- columna1-->
              <div id="column1">
                <div id="descproducto"> 
                  <span class="span"> Descripción del producto:</span>
                  <input id="" type="text" class="inputglobal2">
                </div>

                <div class="cantidadprecio">
                  <div>
                    <span class="span"> Cantidad:</span>
                    <input type="text" class="inputmedio2">

                  </div>
                  <div>
                    <span class="span"> Precio:</span>
                    <input type="text" class="inputmedio2" value="10.000 $">

                  </div>
                </div>

              </div>

              <!-- columna2 -->
              <div id="column2">

                <a class="aceptar">
                  ACEPTAR</a>

                <a class="cancelar modal-close">
                  CANCELAR</a>

              </div>
            </div>


          </div>

        </div>

        <!-- Fila 2  #sindefinir / mayoreo / Ingreso de dinero / salida de dinero / teclado virtual-->
        <div class="five">
          <a class="ancla">
            <i class="material-icons">local_shipping
            </i>Mayoreo
          </a>
        </div>

        <!-- iNGRESO DINERO Trigger -->
        <div class="six">
          <a class="ancla modal-trigger" href="#ingresoDinero">

            <i class="material-icons">arrow_upward</i>
            Ingreso Dinero</a>

          <!--  INGRESO DINERO Modal Structure -->
          <div id="ingresoDinero" class="modal">


            <h6>
              <i class="material-icons orange-text">arrow_upward
              </i> Ingreso de dinero
            </h6>


            <div style="padding-bottom: 0%;" class="row modal-content">
              <div style="font-size:14px;" class="col s7">
                Escriba el monto a ingresar en
                <br> caja:

                <input id="inputIngreso" type="text" class="ancla" value="10.000 $">
              </div>

              <div class="col s5">
                <a id="" class="aceptar">Aceptar</a>

                <a class="cancelar modal-close">Cancelar</a>

              </div>


            </div>
            <div style="font-size:14px; 
                width:90%;margin-left: 14px; padding-top: 10px;padding-bottom: 0;
                margin-top:0; " class="modal-content">
              <span>Registro de entrada del día:</span>
              </br>

              <textarea name="" id="" cols="20" rows="12">
                </textarea>
            </div>
          </div>

        </div>

        <!-- salida DINERO Trigger -->
        <div class="seven">
          <a class="ancla modal-trigger" href="#salidaDinero">
            <i class="material-icons">arrow_downward</i>
            Salida Dinero</a>

          <!--  Salida DINERO Modal Structure -->

          <div id="salidaDinero" class="modal">
            <h6>
              <i class="material-icons orange-text">arrow_upward
              </i> Salida de dinero
            </h6>


            <div style="padding-bottom: 0%;" class="row modal-content">
              <div style="font-size:14px;" class="col s7">
                Monto:

                <input id="inputIngreso" type="text" class="ancla" value="10.000 $">
              </div>

              <div class="col s5">
                <a id="" class="aceptar">Aceptar</a>

                <a class="cancelar modal-close">Cancelar</a>

              </div>


            </div>
            <div style="font-size:14px; 
                width:90%;margin-left: 14px; padding-top: 10px;padding-bottom: 0;
                margin-top:0; " class="modal-content">
              <span>Registro de entrada del día:</span>
              </br>

              <textarea name="" id="" cols="20" rows="12">
                </textarea>
            </div>
          </div>

        </div>


        <div class="eight">
          <a class="ancla" onClick="muestra_oculta('teclado')">
            <i class="material-icons">keyboard</i>
            Teclado Virtual</a>
        </div>

      </div>

      <!-- Resultado búsqueda -->

<div id="resultados"></div>
      <!--Tabla ventas-->


      @foreach ($productos as $producto)       
     
     <p> {{$producto->descripcion}} </p>
      
      @endforeach



      <div class="despliegueInfo">

        <table id="tablaventas">

          <tr>
            <th>Un</th>
            <th>Descripción</th>
            <th>Precio</th>
            <th>-</th>
            <th>+</th>
          </tr>

        
         
        </table>
      </div>
    </main>


    <aside class="sidebar">
      <!--Calculadora-->
      <div id="calculadora">
        <!-- Note that "m4 l3" was added -->

        <!-- resultadocalculadora  -->
        <div>
          <input class="resultadocalculadora" id="result" type="text" value="$42.980" />


          <div style="display: flex;justify-content: center; align-items: center;">
            <!-- Teclado -->
            <div id="teclado" class="center">

              <div class="numeros center">
                <button type="button" value="7" onclick="dis('7')">7</button>
                <button type="button" value="8" onclick="dis('8')">8</button>
                <button type="button" value="9" onclick="dis('9')">9</button>


                <button type="button" value="4" onclick="dis('4')">4</button>
                <button type="button" value="5" onclick="dis('5')">5</button>
                <button type="button" value="6" onclick="dis('6')">6</button>


                <button type="button" value="1" onclick="dis('1')">1</button>
                <button type="button" value="2" onclick="dis('2')">2</button>
                <button type="button" value="3" onclick="dis('3')">3</button>


                <button type="button" value="0" onclick="dis('0')">0</button>
                <button type="button" value="," onclick="dis(',')">,</button>
                <button type="button" value="," onclick="clr()">Borrar</button>

                <button type="button" data-target="cantidad" class="modal-trigger cteclado"
                  value="Cantidad">Cantidad</button>

                <button type="button" data-target="precio" class="modal-trigger pteclado" value="Precio">Precio</button>

                <button type="button" data-target="descuento" class="modal-trigger dteclado" value="Descuento">
                  Descuento</button>


              </div>
              <!-- fin de numeros-->

            </div>

          </div>
          <!-- fin de teclado-->

          <!-- XXXXXXXXXXX    Modales de calculadora XXXXXXXXXXXXXXXXXXXX-->

          <!-- Cantidad Modal Structure -->

          <div id="cantidad" class="modal">
            <div style=" display: flex;
          flex-direction: column;
          align-items: center;">

              <div>
                <h5 class="center">
                  <i class="material-icons orange-text">arrow_upward
                  </i>
                  Cantidad
                </h5>

              </div>
              <h6>Escriba la cantidad de producto:</h6>
              <div style="display: flex;justify-content: space-around; 
          align-items: center;">
                <input type="text" class="inputglobal" value="">
              </div>
              <div style="width: 400px;padding-left: 17px; display: flex;justify-content: space-around; 
align-items: center;" class="row center">

                <a id="" class="aceptar">ACEPTAR</a>



                <a id="" class="cancelar modal-close">CANCELAR</a>


              </div>
            </div>


          </div>

          <!-- precio Modal Structure -->
          <div id="precio" class="modal">
            <div style=" display: flex;
flex-direction: column;
align-items: center;">
              <div>
                <h5 class="center">
                  <i class="material-icons orange-text">arrow_upward
                  </i>
                  Precio
                </h5>

              </div>
              <h6>Ingrese el precio del producto:</h6>
              <div style="display: flex;justify-content: space-around; 
 align-items: center;">
                <input type="text" class="inputglobal" value="">
              </div>

              <div style="width: 400px;padding-left: 17px; display: flex;justify-content: space-around; 
align-items: center;" class="row center">

                <a id="" class="aceptar">ACEPTAR</a>



                <a id="" class="cancelar modal-close">CANCELAR</a>


              </div>

            </div>


          </div>

          <!-- descuento Modal Structure -->
          <div id="descuento" class="modal">
            <div style=" display: flex;
   flex-direction: column;
   align-items: center;">
              <div>
                <h5 class="center">
                  <i class="material-icons orange-text">arrow_upward
                  </i>
                  Descuento
                </h5>

              </div>
              <h6>Ingrese el descuento que será aplicado al monto final del producto:</h6>
              <div style="display: flex;justify-content: space-around; 
     align-items: center;">
                <input type="text" class="inputglobal" value="">
              </div>

              <div style="width: 400px;padding-left: 17px; display: flex;justify-content: space-around; 
  align-items: center;" class="row center">

                <a id="" class="aceptar">ACEPTAR</a>



                <a id="" class="cancelar modal-close">CANCELAR</a>


              </div>

            </div>
          </div>

          <!-- Cobrar -->
          <div style="
                display:flex;
                justify-content: center; 
                align-items: center;
                padding-top: 20px;">

            <!-- cobrar Trigger -->

            <button class="modal-trigger" data-target="cobrar" id="btn-cobrar">
              <span style="font-weight: bold;">COBRAR</span> </button>

            <!--  Cobrar Modal -->
            <div id="cobrar" class="modal">

              <!--primera fila-->
              <div class="contenedorcolumnas">
                <!-- Primera columna-->
                <div id="columna1">
                  <div style="margin-top: 20px;" class="col s12 center">
                    <h5 class="orange-text">Total a cobrar</h5>
                    <h3 style="font-weight: bold;" class="orange-text">$10.000</h3>
                  </div>

                  <div class="cont-mediopago">
                    <!--Medio de pago -->
                    <div onclick="document.getElementById('imgefectivo')
                    src='img/efectivo-naranja.png'"
                      class="mediopago">
                      <img id="imgefectivo" src="img/efectivoblanco.png" alt="">

                    </div>

                    <div onclick="document.getElementById('imgcredito')
                    src='img/creditonaranja.png'" class="mediopago">
                      <img id="imgcredito" src="img/creditoblanco.png" alt="">

                    </div>

                    <div onclick="document.getElementById('imgdebito')
                    src='img/debitonaranja.png'" class="mediopago">
                      <img id="imgdebito" src="img/debitoblanco.png" alt="">

                    </div>

                    <div onclick="document.getElementById('imgtransferencia')
                    src='img/transferencianaranja.png'"
                      class="mediopago">
                      <img id="imgtransferencia" src="img/transferenciablanco.png" alt="">

                    </div>

                  </div>
                  <!-- Pagó Cambio-->
                  <div class="col s12 pagocambio">

                    <h6 class="center black-text">Pagó:</h6>
                    <h6 class="center black-text">Cambio:</h6>
                  </div>

                </div>
                <!-- Segunda columna-->
                <!-- Emitir Documento tributario - Emitir Comprobante - Cancelar  - Ingresar notas-->
                <div id="columna2">


                  <!-- F1 - Emitir documento tributario -->

                  <a id="" href="#emitirdoc" style="margin-top: 30px;" class="modal-trigger ancla">
                    F1-Emitir documento tributario</a>
                  <!--  F1 - Emitir documento tributario Modal -->
                  <div id="emitirdoc" class="modal">
                    <h3>emitir documento</h3>
                  </div>
                  <!-- Emitir comprobante-->
                  <a href="#emitcomp" class="modal-trigger ancla">
                    Emitir Comprobante</a>
                  <!-- emitir comprobante Modal -->
                  <div id="emitcomp" class="modal">
                    <h3>emitir comprobante</h3>
                  </div>
                  <!-- cancelar  -->
                  <a class="modal-close ancla">
                    <i class="material-icons left red-text">close</i>
                    Cancelar </a>
                  <!-- iNGRESO notas  -->

                  <a href="#emitirnotas" class="modal-trigger ancla">
                    <i class="material-icons left green-text">arrow_upward</i>
                    Ingresar Notas</a>


                  <!--  INGRESO notas Modal -->
                  <!--  Ingresar Notas Modal Structure -->
                  <div id="emitirnotas" class="modal animate__animated animate__fadeInLeft animate__faster">

                    <div style="display:flex;flex-direction: column; justify-content: center;align-items: center;">

                      <h6>Ingrese las notas o comentarios de esta venta:</h6>
                      <textarea class="inputglobal" value="">

                        </textarea>


                      <div style="padding: 0 14px; display: flex;justify-content: space-between; 
                  align-items: center;">


                        <a id="" class="cancelar modal-close">CANCELAR</a>

                        <a id="" class="aceptar">ACEPTAR</a>

                      </div>

                    </div>

                  </div>



                  <div style="margin-top: 10px; margin-bottom: 60px;" class="col s12">
                    <h6 class="center white-text">Total de artículos:</h6>
                    <h4 class="center white-text">8</h4>
                  </div>

                </div>
              </div>



            </div>

          </div>


        </div>

      </div>
    </aside>


  </div>



  <script>
    //keyup

    window.addEventListener("load", function(){
      document.getElementById('inputBuscar').addEventListener("keyup", function(){
   
 //  console.log(document.getElementById('inputBuscar').value)   })


 fetch(`/ventas/buscador?inputBuscar=${document.getElementById("inputBuscar").value}`,
 {  method:'get' })
     .then(response => response.text() )
     .then(html => {
       document.getElementById("resultados").innerHTML += html
     })
     .catch(error => console.log(error))
      })
  
    });
  </script>


  <!--  Scripts-->
  <script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>

  <!-- Compiled and minified JavaScript -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/js/materialize.min.js"></script>
    
  <!-- Scripts -->
     <script src="{{ asset('js/index.js') }}" defer></script>



</body>

</html>