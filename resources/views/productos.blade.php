
      <!DOCTYPE html>
      <html lang="en">
      
      <head>
        <meta charset="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta http-equiv="X-UA-Compatible" content="ie=edge" />
        <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet" />
      
        <title>Punto de Venta</title>
      
        <!-- Font Awesome -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css" />
      
        <!--Material Design-->
      
        <script src="https://unpkg.com/material-components-web@latest/dist/material-components-web.min.js"></script>
      
        <link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons" />
        <!--Materialize-->
        <!-- Compiled and minified CSS -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css" />
      
      
          <!-- Styles -->
          <link href="{{ asset('css/modales.css') }}" rel="stylesheet">
          <link href="{{ asset('css/estilos.css') }}" rel="stylesheet">
          <link href="{{ asset('css/responsive.css') }}" rel="stylesheet">
          <link href="{{ asset('css/configuracion.css') }}" rel="stylesheet">
      
      
      </head>
      
      <body>
        <div class="contenedor">
          <header class="header" style="padding-top: 0;padding-left:0px;padding-right: 0; 
              box-shadow: 1px 1px #999999;">
      
      
            <!-- Dropdown Structure -->
            <ul id="dropdown1" class="dropdown-content">
              <li><a href="#!">Perfil</a></li>
              <li><a class="modal-trigger" href="#cerrarelturno">Cerrar turno</a></li>
              <li class="divider"></li>
              <li>
              
                  <a class="dropdown-item" >
                  Cerrar sesión
               </a>
      
               
              </li>
            </ul>
            <nav style="padding: 10px;
                  box-shadow: none;
                  background-color: transparent;">
              <div class="nav-wrapper">
                <a href="#!" class="brand-logo center"
                  style="display:flex;text-align: center;justify-content: center;align-items: center;"><svg width="419"
                    height="80" viewBox="0 0 419 80" fill="none" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="419" height="80" fill="url(#pattern0)" />
                    <defs>
                      <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0" transform="scale(0.00214592 0.011236)" />
                      </pattern>
                      <image id="image0" width="419" height="80"
                        xlink:href="data:image/png;base64," />
                    </defs>
                  </svg></a>
                <a href="#" data-target="mobile-demo" style="display: block;" class="sidenav-trigger">
      
                  <i class="material-icons grey-text" style="font-size: 30px;">menu</i></a>
                <ul class="right">
      
                  <!-- Dropdown Trigger -->
                  <li style="color: grey;">
                    <a class="dropdown-trigger" href="#!" data-target="dropdown1"
                      style="display: flex;justify-content: space-around;color:gray">
                      <i style="font-size: 20px; margin-right: 0px;" class="material-icons grey--text">account_circle</i>
                      
                      <i class="material-icons">arrow_drop_down</i></a></li>
                </ul>
              </div>
            </nav>
      
            <ul class="sidenav nav-menu" id="mobile-demo">
              <li>
                <a class="sidenav-close" style="width:32%;margin-bottom:10px" id="cerrarnavegacion"><i
                    class="material-icons">keyboard_backspace</i>
                </a>
              </li>
              <li>
                <a href="ventas" class="">Ventas<i class="material-icons">check</i>
                </a>
              </li>
              <li>
                <a href="clientes" class="">Clientes<i class="material-icons">people_outline</i></a>
              </li>
              <li>
                <a href="productos" class="">Productos<i class="material-icons">local_offer</i></a>
              </li>
              <li>
                <a href="inventario" class="">Inventario<i class="material-icons">assignment_turned_in</i></a>
              </li>
              <li>
                <a href="cortedeldia" class="">Corte<i class="material-icons">book_online</i></a>
              </li>
              <li>
                <a href="mantenimiento" class="">Mantenimiento<i class="material-icons">build</i></a>
              </li>
              <li>
                <a href="configuracion" class="">Configuración<i class="material-icons">miscellaneous_services</i></a>
              </li>
      
            </ul>
      
            <!-- cerrarelturno Modal Structure -->
            <div id="cerrarelturno" class="modal">
      
              <h5><strong>¿Seguro que desea cerrar el turno?</strong></h5>
      
              <div style="display: flex;justify-content: space-evenly;align-items: center;">
                <a id="" class="aceptar">SI</a>
                <a id="" class="modal-close cancelar">NO</a>
      
              </div>
      
            </div>
          </header>
      
      
      
        <main class="contenido">

            <!-- Fila 1-->
            <div class="opcionesproductos">

                <div style="margin-left:0; margin-bottom: 10px; width: 100%;">
                    <h5>Productos</h5>

                </div>

                

                        <!--nuevo Producto-->
                    <div class="aproduct">
                        <a class="ancla modal-trigger" href="#nuevoproducto">
                            <i class="material-icons">
                                fiber_new</i>Nuevo</a>

     <!--  nuevo Producto Modal Structure -->
      <div id="nuevoproducto" class="modal">

         
        <form action="{{ url('/productos')}}" method="post" enctype="multipart/form-data">
           {{csrf_field()}}

      <h5> <i class="material-icons orange-text">fiber_new </i> Nuevo Producto</h5>
      
        <div>
        Código de barras
         <input class="inputglobal" name="codigodebarra" type="text">
    
          </div>
       <div>
    Descripción <input type="text" name="descripcion" class="inputglobal">
    </div>
    
   <div>
  Se vende<input type="text" name="sevende" class="inputglobal">
   </div>

  <div>
   Precio Costo<input type="text" name="preciocosto" class="inputglobal">
   </div>
      
      <div >
      Precio Venta<input  type="text" class="inputglobal"
      name="precioventa" >
    </div>


     <div>
       Precio Mayoreo<input  type="text" class="inputglobal" 
      name="preciomayoreo" >
     </div>
      
                               
	  
            <div> Departamento
          <input  type="text" class="inputglobal" name="departamento" >
                                
                </div>  


    <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
                 <button type="submit" class="guardarcrear">
                                        Crear producto</button>
      
                   </div>
      
                              </form>
                          </div>
      

                    </div>

                
                              <!--editar Producto-->
                    <div class="aproduct">
                
                        <a class="ancla modal-trigger" href="#editarproducto">
                            <i
                                class="material-icons">
                                create</i>Editar</a>

                                
                    <!--  editar Producto Modal Structure -->
                    <div id="editarproducto" class="modal">

                        <h5>
                            <i class="material-icons orange-text"
                                >create
                            </i> Modificar Producto
                        </h5>

           <div style="display: flex;justify-content: space-between;align-items: center;">
                          Código de barras
                            <input class="inputglobal" type="text">
                        </div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                          Descripción</span> <input type="text" class="inputglobal">
                        </div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Se vende<input id="" type="text" class="inputglobal"></div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Precio Costo<input id="" type="text" class="inputglobal"></div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Precio Venta<input id="" type="text" class="inputglobal"></div>

                        <div
                            style="display: flex;justify-content: space-between;align-items: center;">
                            Precio Mayoreo<input id="" type="text" class="inputglobal"></div>

                        <div
                            style="display: flex;justify-content: space-between;align-items: center;">
                            Departamento<input id="" type="text" class="inputglobal"></div>



                            <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
                              <a id="" class="guardarcrear">
                                  Guardar</a>
                    </div>

                    </div>
                    </div>

                              <!--eliminar Producto-->
                    <div class="aproduct">
                        <a class="modal-trigger ancla" href="#eliminarproducto"
                            >
                            <i
                                class="material-icons">
                                close</i>Eliminar</a>

                                   <!--  Modal Structure -->
                    <div  id="eliminarproducto"  class="modal" >

                    <div>
                        <h5 class="center">
                            Eliminar Producto
                        </h5>

                    </div>

                    <div style="text-align: center;">                    
                    <h6>Inserte el código del producto que desea eliminar:</h6>
                    <input type="text" class="inputglobal" value="">

                </div>


                    <div style="width: 400px;padding-left: 17px; display: flex;justify-content: space-around; 
      align-items: center;" class="row center">

                        <a id="" class="aceptar">ACEPTAR</a>



                        <a id="" class="cancelar modal-close">CANCELAR</a>


                    </div>




                </div>
                    </div>

                                <!-- Departamentos-->
                                <div  class="rotar" >
                       
                                  <a class="ancla" onclick="despliegueDeptos()" 
                                     >Departamentos<i id="deptogirar"
                                          class="material-icons" style="margin-left: 1em;">
                                          expand_more</i></a>
          
                              </div>
          
                                            <!-- Promociones -->
                              <div  class="rotar">
                                  <!-- promociones Trigger -->
                                  <a class="ancla" onclick="desplieguePromos()"
                                     >Promociones<i id="promogirar"
                                      class="material-icons" style="margin-left: 1em;">
                                      expand_more</i></a>
          
                              </div>
 


                </div>

                
                    <!-- Departamentos despliegue-->

                <div id="deptos" class="invisible">


                       <div style="margin-top: 0; width: 100%;">
                           <h5>Departamentos</h5>
                       </div>

                       <!--editar Departamentos-->
                       <div class="promodepto">
                           <a href="#nuevodepartamento" class="anclanaranja modal-trigger"><i class="material-icons">
                            fiber_new</i>Nuevo departamento</a>
                       </div>

                            <!--  nuevo Producto Modal Structure -->
                            <div id="nuevodepartamento" class="modal">

   <form action="{{url('/departamentos')}}" method="post" enctype="multipart/form-data">
   {{csrf_field()}}


                                <div>
                           <h5>
                              <i class="material-icons orange-text"
                             style="font-size:30px ;margin-right: 10px;">fiber_new
                              </i> Nuevo Departamento
                                    </h5>
    
                                </div>
    
                                <div
                                    style="display: flex;flex-direction: column; justify-content: center;align-items: center;">
                                    <div>
                                        <span >Nombre del departamento:</span>
                                    </div>
                                    <div>
                                          <input class="inputglobal" name="nombredepartamento" type="text">
    
                                    </div>
                                    
    
                                </div>
    
    
                                <!--crear-->
             <div style="margin-top:5px;display: flex;justify-content: center;align-items: center;">
                            
              <button type="submit" class="guardarcrear">
                               Crear</button>
  
                              </div>

   </form>
    
    
                            </div>
    

                       <!-- eliminar  Departamentos Trigger -->
                       <div class="promodepto">
                           <a href="#eliminardepartamento" class="anclanaranja modal-trigger">
                               <i class="material-icons">
                                   close</i>Eliminar departamento</a>

                                       <!-- eliminar Departamento Modal Structure -->
                    <div id="eliminardepartamento" class="modal">


                        <h5><strong>¿Seguro que desea eliminar el departamento?</strong></h5>


                        <div style="display: flex;justify-content: space-evenly;align-items: center;">
                            <a id="" class="aceptar">SI</a>
                            <a id="" class="modal-close cancelar">NO</a>

                        </div>

                    </div>
                   
                       </div>

                    <!-- Tabla Departamento -->
                    <div class="despliegueInfo">
                      <table>

                          <tr>
                            <th> </th>
                              <th>ID</th>
                              <th>Nombre del departamento</th>

                              <th>Acciones</th>
                          </tr>
                          @foreach($departamentos as $departamento)
                          <tr class="">
                            <td>  
                              <input type="checkbox" id="vehicle1" name="vehicle1" value=""
                              style="background-color: yellow; width:180px">
                            
                            
                            </td>
                              <td>{{$departamento->id}}</td>
                              <td>{{$departamento->nombredepartamento}}</td>
                              <td>
                <form action="{{url('/departamentos/'.$departamento->id)}}" method="post" style="padding-top: 0">
                         {{csrf_field()}}
                                {{method_field('DELETE')}}
           
                                <button type="submit" onclick="return confirm('¿eliminar?');">
                                  <i style="color:red;" class="material-icons">
                                    delete </i></button>
                         </form>
                        </td>
                          </tr>
                          @endforeach
                        
                      </table>
                  
                  </div>



                   </div>

                      <!-- Promociones despliegue-->
                      <div  id="promos" class="invisible">
      
      
                             <div style="margin-left: 1%; margin-top: 0; width: 100%;">
                                 <h5>Promociones</h5>
                             </div>
      
                             <!-- nueva promoción-->
                             <div class="promodepto">
                                 <a href="#nuevapromocion" class="anclanaranja modal-trigger"><i class="material-icons">
                                    fiber_new</i>Nueva Promoción</a>
                             </div>

                                   <!--  nueva Promoción Modal Structure -->
                    <div id="nuevapromocion" class="modal">

                        <h5 class="center"
                            style="display: flex;justify-content: center;align-items: center">
                            <i class="material-icons orange-text"
                                style="font-size:30px ;margin-right: 10px;">fiber_new
                            </i> Nueva Promoción
                        </h5>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Nombre
                        <input class="inputglobal" type="text">
                        </div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Código de barras
                             <input type="text" class="inputglobal">
                        </div>

                        <div style="display: flex;justify-content: space-between;align-items: center;">
                            Cantidad<input id="" type="text" class="inputglobal"></div>

                            <!--  DESDE  HASTA-->

                        <div style="display: flex;justify-content: space-between;width:100%;margin:10px 0;">
                         
                          <div id="desdehasta">
                              
                              <span style="margin-right:10px">Desde</span>
                                
                              <input id="" type="date" class="inputmedio">
                           
                              <span style="margin:0 10px;">Hasta</span>
                              <input id="" type="date" class="inputmedio2">
                              
                            </div>


                        </div>

                        <div
                            style="display: flex;justify-content: space-between;align-items: center;">
                            Precio<input type="text" class="inputglobal"></div>

                            <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
                              <a class="guardarcrear">
                                  Crear</a>
                          </div>
                    </div>
      
                             <!-- eliminar  cliente Trigger -->
                             <div class="promodepto">
             <a href="#eliminarpromocion" class="anclanaranja modal-trigger">
                                     <i class="material-icons">
                                         close</i>Eliminar Promoción</a>
                                      
                                         <!--modal eliminar promoción-->

               <div id="eliminarpromocion" class="modal" >

                                <h6><strong>¿Seguro que desea eliminar la promoción?</strong></h6>
                                    
                 <div style="display: flex;justify-content: space-evenly;align-items: center;">
                                 <a id="" class="aceptar">SI</a>
                                    
                                      <a id="" class="modal-close cancelar">NO</a>
                                    
                                                            </div>
                                    
                                                        </div>

                            
                             </div>
      
                         </div>

            <!--Fila 2-->



            <div>

              <div  class="despliegueInfo">
                <table>
                    <tr>
                        <th>Código</th>
                        <th>Título del producto</th>
                        <th>Precio</th>
                        <th>Acciones</th>
                    </tr>

                
                    @foreach ($productos as $producto)       
                    <tr>
                    
                    <td>{{$producto->codigodebarra}}/ {{$producto->departamento}}</td>
                    <td>{{$producto->descripcion}}</td>
                    <td>$ {{$producto->precioventa}}</td>
                  
                    <td style="display: flex;justify-content:space-evenly;align-items:center">
                   
                    
                    <a href="{{url('/productos/'.$producto->id.'/edit')}}">
                      <i style="color:green;" class="material-icons">
                        edit </i>
                    </a>
                    |
                
             <form action="{{url('/productos/'.$producto->id)}}" method="post" style="padding-top: 0">
                    {{csrf_field()}}
                    {{method_field('DELETE')}}


                    <button type="submit" onclick="return confirm('¿eliminar?');">
                      <i style="color:red;" class="material-icons">
                        delete </i></button>
             </form>
                    </td>
                    </tr>
                    @endforeach


           </table>

          
              </div>
      
              
            </div>
            <div style="display:flex;justify-content:center;align-items:center">

             <div style="display:flex;justify-content:space-around;align-items:center;
             background-color:white"> {{$productos->links()}}</div>
            </div>


          <form action="" method="post" enctype="multipart/form-data"
            style=" display:flex; justify-content: flex-end;align-items: center;">
            @csrf
            <input type="file" name="file" id="file" style="margin-right: 10px;" >
              <button type="submit" id="importar" href="#arreglar">

                <i style="padding-right: 4px; color:green;" class="material-icons">
                    file_download </i> Importar desde excel
                 
                  </button>
                  
           
        </form>
               
                </div>


    </main>

    <aside class="sidebar">


    </aside>



    </div>


    <!--  Scripts-->
    <script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>

    <!-- Compiled and minified JavaScript -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/js/materialize.min.js"></script>
    
  <!-- Scripts -->
  <script src="{{ asset('js/index.js') }}" defer></script>



</body>

</html>