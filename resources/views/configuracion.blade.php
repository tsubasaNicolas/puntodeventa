
      <!DOCTYPE html>
      <html lang="en">
      
      <head>
        <meta charset="UTF-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
        <meta http-equiv="X-UA-Compatible" content="ie=edge" />
        <link href="https://fonts.googleapis.com/css?family=Roboto" rel="stylesheet" />
      
        <title>Punto de Venta</title>
      
        <!-- Font Awesome -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css" />
      
        <!--Material Design-->
      
        <script src="https://unpkg.com/material-components-web@latest/dist/material-components-web.min.js"></script>
      
        <link rel="stylesheet" href="https://fonts.googleapis.com/icon?family=Material+Icons" />
        <!--Materialize-->
        <!-- Compiled and minified CSS -->
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css" />
      
      
          <!-- Styles -->
          <link href="{{ asset('css/modales.css') }}" rel="stylesheet">
          <link href="{{ asset('css/estilos.css') }}" rel="stylesheet">
          <link href="{{ asset('css/responsive.css') }}" rel="stylesheet">
          <link href="{{ asset('css/configuracion.css') }}" rel="stylesheet">
      
      
      </head>
      
      <body>
        <div class="contenedor">
          <header class="header" style="padding-top: 0;padding-left:0px;padding-right: 0; 
              box-shadow: 1px 1px #999999;">
      
      
            <!-- Dropdown Structure -->
            <ul id="dropdown1" class="dropdown-content">
              <li><a href="#!">Perfil</a></li>
              <li><a class="modal-trigger" href="#cerrarelturno">Cerrar turno</a></li>
              <li class="divider"></li>
              <li>
              
                  <a class="dropdown-item" href="{{ route('logout') }}"
                  onclick="event.preventDefault();
                                document.getElementById('logout-form').submit();">
                  Cerrar sesión
               </a>
      
               <form id="logout-form" action="{{ route('logout') }}" method="POST" style="display: none;">
                   @csrf
               </form>
              </li>
            </ul>
            <nav style="padding: 10px;
                  box-shadow: none;
                  background-color: transparent;">
              <div class="nav-wrapper">
                <a href="#!" class="brand-logo center"
                  style="display:flex;text-align: center;justify-content: center;align-items: center;"><svg width="419"
                    height="80" viewBox="0 0 419 80" fill="none" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink">
                    <rect width="419" height="80" fill="url(#pattern0)" />
                    <defs>
                      <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                        <use xlink:href="#image0" transform="scale(0.00214592 0.011236)" />
                      </pattern>
                      <image id="image0" width="419" height="80"
                        xlink:href="data:image/png;base64," />
                    </defs>
                  </svg></a>
                <a href="#" data-target="mobile-demo" style="display: block;" class="sidenav-trigger">
      
                  <i class="material-icons grey-text" style="font-size: 30px;">menu</i></a>
                <ul class="right">
      
                  <!-- Dropdown Trigger -->
                  <li style="color: grey;">
                    <a class="dropdown-trigger" href="#!" data-target="dropdown1"
                      style="display: flex;justify-content: space-around;color:gray">
                      <i style="font-size: 20px; margin-right: 0px;" class="material-icons grey--text">account_circle</i>
                      <span id="usernav"> {{ Auth::user()->name }}</span>
                      <i class="material-icons">arrow_drop_down</i></a></li>
                </ul>
              </div>
            </nav>
      
            <ul class="sidenav nav-menu" id="mobile-demo">
              <li>
                <a class="sidenav-close" style="width:32%;margin-bottom:10px" id="cerrarnavegacion"><i
                    class="material-icons">keyboard_backspace</i>
                </a>
              </li>
              <li>
                <a href="ventas" class="">Ventas<i class="material-icons">check</i>
                </a>
              </li>
              <li>
                <a href="clientes" class="">Clientes<i class="material-icons">people_outline</i></a>
              </li>
              <li>
                <a href="productos" class="">Productos<i class="material-icons">local_offer</i></a>
              </li>
              <li>
                <a href="inventario" class="">Inventario<i class="material-icons">assignment_turned_in</i></a>
              </li>
              <li>
                <a href="cortedeldia" class="">Corte<i class="material-icons">book_online</i></a>
              </li>
              <li>
                <a href="mantenimiento" class="">Mantenimiento<i class="material-icons">build</i></a>
              </li>
              <li>
                <a href="configuracion" class="">Configuración<i class="material-icons">miscellaneous_services</i></a>
              </li>
      
            </ul>
      
            <!-- cerrarelturno Modal Structure -->
            <div id="cerrarelturno" class="modal">
      
              <h5><strong>¿Seguro que desea cerrar el turno?</strong></h5>
      
              <div style="display: flex;justify-content: space-evenly;align-items: center;">
                <a id="" class="aceptar">SI</a>
                <a id="" class="modal-close cancelar">NO</a>
      
              </div>
      
            </div>
          </header>


          
  </div>
  <div class="center">
    <div class="contconfig">
      <h5 id="configtitulo">Configuración</h5>
      <!-- General -->
      <div id="general">

        <div class="title">
          <h6>General</h6>
        </div>


        <!--  opcionesHabilitadas -->
        <a href="#opcioneshabilitadas" class="modal-trigger anclaconfig">
          <img src="img/opcioneshabilitadas.png" alt="" class="imgconfig">
          <span>Opciones habilitadas</span>
        </a>

        <!-- opcionesHabilitadas Modal -->
        <div id="opcioneshabilitadas" class="modal">
          <div style="text-align:center">
            <h6>Opciones Habilitadas</h6>
          </div>
          <div>

            <input type="checkbox" class="filled-in left" checked="checked" />

            <span><strong class="black-text">
                Utilizar inventarios para mis productos</strong></span>
          </div>

          <div>
            <p>Si usas inventario, tus productos tendrán cantidades limitadas en venta podrás llevar un control de
              cuánto tienes, cuándo y cuanto se vende.
            </p>
            <p>Si actualmente no usas inventario, puedes no usarlo y posteriormente activarlo.</p>
          </div>


          <div>

            <input type="checkbox" class="filled-in left" checked="checked" />

            <span><strong class="black-text">
                Deseo habilitar crédito a mis clientes.
              </strong></span>

          </div>

          <div>
            <p>Activa esta opción para dar de alta a clientes y poder ofrecer ventas a crédito, recibir abonos y
              liquidar adeudos.
            </p>


          </div>


          <div>

            <input type="checkbox" class="filled-in left" checked="checked" />

            <span><strong class="black-text">
                Habilitar venta de producto común
              </strong></span>

          </div>

          <div>
            <p>Deseas activar la opción de venta de "Producto Común", con la cual puedes vender artículos que NO están
              en la base de datos al momento de hacer una venta, por ejemplo: chicles, dulces, artículos esporádicos,
              etc.
            </p>
          </div>


          <div>
            <input type="checkbox" class="filled-in left" checked="checked" />
            <span><strong class="black-text">
                Calcular automáticamente precio de venta con el margen de ganancia del
                <input type="text" class="porcentaje"> % al crear,
                modificar o ingresar inventario a los productos.
              </strong></span>
          </div>

          <div>

            <input type="checkbox" class="filled-in left" checked="checked" />

            <span><strong class="black-text">
                Habilitar redondeo de centavos a cantidad cerrada.

              </strong></span>
            <div style="width:100%;margin-top:10px;">
              <input type="text" class="inputglobal" style="width: 300px;">
            </div>

          </div>

        </div>

        <!--  Cajeros-->
        <a href="#cajeros" class="modal-trigger anclaconfig">
          <img src="img/cajeros.png" alt="" class="imgconfig">
          <span>Cajeros</span>

        </a>

        <!--  Cajeros Modal-->
        <div id="cajeros" class="modal">
          <h5 style="text-align: center;">Cajeros</h5>


          <div id="contc">
            <div class="c1">

              <div class="input-field" id="search">
                <input type="search" style="background-color: white;
                      border-bottom: none;text-align: center;">
                <label>
                  <i class="material-icons center orange-text center" style="margin-left: 10px;">
                    search</i></label>
              </div>

              <table>


                <tr class="row white">
                  <td> <i class="material-icons center orange-text center">
                      account_circle</i></label>
                  </td>
                  <td>Diana Herrera Montenegro</td>
                  <td></td>
                  <td></td>
                  <td></td>
                </tr>
                <tr class="row white">
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>


                </tr>
                <tr class="row white">
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>

                </tr>
                <tr class="row white">
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>
                  <td class="row white"></td>
                </tr>



              </table>

            </div>



            <div class="c2">

              <div style="display: flex;justify-content:space-between">


                <a class="modal-trigger anclanaranjaconfig">
                  <i class="material-icons left">person_add_alt_1</i>
                  Nuevo Cajero</a>

                <a href="#bajacajero" class="modal-trigger anclanaranjaconfig">
                  <i class="material-icons left">person_remove</i>
                  Dar de baja</a>

                <!-- dar de baja  Modal Structure -->
                <div id="bajacajero" class="modal">

                  <h6><strong>¿Seguro que desea dar de baja al cajero?</strong></h6>

                  <div style="display: flex;justify-content: space-evenly;align-items: center;">
                    <a id="" class="aceptar">SI</a>
                    <a id="" class="modal-close cancelar">NO</a>

                  </div>

                </div>


              </div>

              <!-- trigger funciones del cajero-->




              <div style="padding:10px; margin-top: 0;">
                <h6 style="text-align: center;">Diana Herrera</h6>
                <div class="col s4"> Usuario:</div>

                <div class="col s8">
                  <input id="" type="text" class="inputglobal"></div>

              </div>
              <div style="padding:10px; margin-top: 0;">

                <div class="col s4"> Clave:</div>

                <div class="col s8"><input id="" type="text" class="inputglobal">

                </div>
              </div>



              <div style="padding:10px;">
                <div class="col s4"> Nombre Completo:</div>

                <div class="col s8">
                  <input id="" type="text" class="inputglobal"></div>

              </div>



              <div class="col s12 center">

                <div class="col s12" style="padding: 0; margin-top: 10px;">
                  <a href="#funcionescajero" class="modal-trigger anclanaranjaconfig">

                    <i class="material-icons left">cloud</i>
                    Funciones del Cajero</a>


                </div>

                <!-- trigger funciones del cajero-->




              </div>



            </div>



          </div>








        </div>



        <a href="#bbdd" class="modal-trigger anclaconfig">
          <img src="img/basededatos.png" alt="" class="imgconfig">
          <span>Base de datos</span>

        </a>

        <!--  BBDD Modal Structure -->
        <div id="bbdd" class="modal">


          <h5 style="text-align: center; margin-bottom:0; padding-bottom:0;">Base de datos</h5>


          <div class="modal-content" style="margin-top: 0;padding-top:0;">
            <h5>Exportar base de datos</h5>

            <p>Si cuentas con otro negocio con IMPOTEC POS, esta opción permite exportar la base de datos y permitir
              re-usar los productos de la base de datos actual.</p>

            <p>NOTA: Se exportará la base de datos con los productos con inventario ilimitado, los mismos departamentos
              y
              unicamente con el usuario administrador (no se transfieren cantidades en inventario, cajeros adicionales,
              clientes, promociones ni ventas).
            </p>


            <a class="anclanaranjaconfig">
              <i class="material-icons left orange-text">redo</i>
              Exportar base de datos</a>

            <h5>Regenerar totales de tickets</h5>

            <p>La siguiente opción permite regenerar a los totales de todas las ventas (ejecuta esta opción sólo
              si notas algún error de cálculo al visualizar un ticket).
            </p>

            <a class="anclanaranjaconfig">
              <i class="material-icons left orange-text">beenhere</i>
              Regenerar totales de tickets</a>

            <h5>Soporte base de datos</h5>

            <div id="contancla">
              <a class="anclanaranjaconfig">
                <i class="material-icons left orange-text">check_circle_outline</i>
                Verificar base de datos</a>

              <a class="anclanaranjaconfig">
                <i class="material-icons left orange-text">handyman</i>
                Leer archivo de mantenimiento</a>

              <a class="anclanaranjaconfig">
                <i class="material-icons left orange-text">forward</i>
                Reinicializar base de datos</a>


            </div>



          </div>

        </div>

        <a href="#foliotickets" class="modal-trigger anclaconfig">
          <img src="img/facturacion.png" alt="" class="imgconfig">
          <span>Facturación</span>

        </a>

        <!--  nuevo producto Modal Structure -->
        <div id="foliotickets" class="modal">
          <div class="modal-content">

            <h4 class="center">
              Folio de tickets
            </h4>

          </div>

          <div class="col s12">
            <div style="display: flex; flex-direction: column;justify-content: center; align-items: center;">
              <p style="text-align: center;">Puedes configurar aquí el manejo de los folios de las ventas del programa
              </p>

              <div style="display:flex;justify-content:center; align-items:center; margin:10px 0px;">Nombre:</div>


              <div style="display: flex; justify-content: center; align-items: center;">
                <input type="text" class="ancla"></div>

            </div>




          </div>



        </div>

        <!-- modificar  averiguar-->
        <a href="#modificarmoneda" class="modal-trigger anclaconfig">
          <img src="img/modificarfolios.png" alt="" class="imgconfig">
          <span>Modificar folios</span>

        </a>

        <!--  Modificar Modal Structure -->
        <div id="modificarmoneda" class="modal">

          <h5 class="center"
            style="margin-top: 0px;display: flex;justify-content: center;align-items: center;width:100%">
            <i class="material-icons orange-text" style="font-size:50px ;margin-right: 10px;">account_circle
            </i> Símbolo de moneda
          </h5>

          <div class="contenidosimbolo">
            <div style="display: flex;justify-content: space-between;align-items: center;">
              <span style=" margin-left: 16px;">Símbolo de moneda</span>

              <input class="inputglobal" type="text">

            </div>


            <div style="display: flex;justify-content: space-between;align-items: center;">

              <span style=" margin-left: 16px;">Separador de miles</span> <input type="text" class="inputglobal">
            </div>

            <div style="display: flex;justify-content: space-between;align-items: center;
          margin-left: 16px;">
              Separador decimal<input id="" type="text" class="inputglobal"></div>

            <div style="display: flex;justify-content: space-between;align-items: center;
          margin-left: 16px;">
              Número de decimales<input id="" type="text" class="inputglobal"></div>

          </div>

          <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
            <a id="" class="guardarcrear">
              Guardar</a>


          </div>



        </div>

      </div>



      <!-- Personalización -->
      <div id="personalizacion">

        <div class="title">
          <h6>Personalización</h6>
        </div>

        <!--personalizacion de tickets-->

        <a href="#persoticket" class="modal-trigger anclaconfig">
          <img src="img/ticket.png" alt="" class="imgconfig">
          <span>Ticket</span>

        </a>

        <!--  nuevo Producto Modal Structure -->
        <div id="persoticket" class="modal">
          <h6 style="text-align: center;">
            Personalización de ticket de venta
          </h6>

          <div style="width:100%;display: flex;">


            <div class="conttextarea">

              <textarea>  </textarea>
            </div>

            <div style="padding: 10px">

              <label>

                <input type="checkbox" class="orange" checked="" />
                <span>Incluir precio unitario en la impresión de ticket.</span>
              </label>

              <label>

                <input type="checkbox" class="orange" checked="" />
                <span>Incluir descripción completa (varios renglones para la descripción).</span>
              </label>


            </div>

          </div>

        </div>

        <a href="#formadepago" class="modal-trigger anclaconfig">
          <img src="img/formatodepago.png" alt="" class="imgconfig">
          <span>Formas de pago</span>
        </a>
        <!--  Forma de pago Modal Structure -->

        <!--  Forma de pago Modal Structure -->
        <div id="formadepago" class="modal">
          <h5 class="center" style="margin-top: 0;">
            Formas de pago
          </h5>

          <div class="row modal-content">
            <div class="col s12">
              <h6 class="left" style="margin-left:10px;">Efectivo</h6>

              <div class="col s12 left">

                <input type="checkbox" />


                <span><strong class="black-text">Utilizar inventarios para mis
                    productos</strong></span> </label>

              </div>

              <div class="col s12">
                <img src="img/efectivoblanco.png" style="margin-top:5px;" class="left" alt="" width="50px"
                  height="50px">
              </div>
            </div>
            <div class="col s12">

              <div class="col s12">
                <h6 class="left">Dólares americanos</h6>
              </div>

              <div class="col s12">

                <input type="checkbox" class="filled-in left" checked="checked" />

                <span><strong class="black-text">Utilizar inventarios para mis
                    productos</strong></span>

              </div>

              <div class="col s12">
                <img src="img/debitoblanco.png" style="margin-top:5px;" class="left" alt="" width="50px" height="50px">
              </div>

            </div>


            <div class="col s12">

              <div class="col s12">
                <h6 class="left">Tarjeta de crédito</h6>
              </div>

              <div class="col s12">


                <input type="checkbox" class="filled-in left" checked="checked" />

                <span><strong class="black-text">Utilizar inventarios para mis
                    productos</strong></span>

              </div>

              <div class="col s12">
                <img src="img/creditoblanco.png" style="margin-top:5px;" class="left" alt="" width="50px" height="50px">
              </div>

            </div>


            <div class="col s12" style="margin-bottom: 20px;">

              <div class="col s12">
                <h6 class="left">Vales de despensa</h6>
              </div>

              <div class="col s12">
                <input type="checkbox" class="filled-in left" checked="checked" />
                <span><strong class="black-text">Utilizar inventarios para mis
                    productos</strong></span>

              </div>



            </div>



          </div>

        </div>

        <a href="#impuestos" class="modal-trigger anclaconfig">
          <img src="img/impuestos.png" alt="" class="imgconfig">
          <span>Impuestos</span>

        </a>
        <!--  nimpuestos Modal Structure -->
        <div id="impuestos" class="modal">
          <h4 class="center">Impuestos</h4>
          <div style="text-align: center;">
            <input type="checkbox" class="filled-in left" checked="checked" />
            <span class="black-text"> Mis productos manejan impuestos (iva, etc)</span>
          </div>
        </div>



        <a href="#udemedidas" class="modal-trigger anclaconfig">
          <img src="img/simbolodemoneda.png" alt="" class="imgconfig">
          <span>Símbolo de moneda</span>

        </a>
        <!--  Modificar Modal Structure -->
        <div id="udemedidas" class="modal">

          <h5 class="center" style="margin-top: 0px;display: flex;justify-content: center;
                  align-items: center;width:100%">
            <i class="material-icons orange-text" style="font-size:50px ;margin-right: 10px;">account_circle
            </i> Unidades de medidas
          </h5>

          <p style="margin:20px; text-align:center;">Puedes personalizar las unidades de medida que se usan al imprimir
            el ticket:</p>

          <div class="contenidosimbolo">
            <div style="display: flex;justify-content: space-between;align-items: center;">
              <span style=" margin-left: 16px;">Unidad para productos "a granel" mayores a uno(1)
              </span>
              <input class="inputglobal" type="text">

            </div>


            <div style="display: flex;justify-content: space-between;align-items: center;">

              <span style=" margin-left: 16px;">Unidad para productos "a granel" mayores a uno(1)
              </span>
              <input type="text" class="inputglobal">
            </div>



            <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
              <a id="" class="guardarcrear">
                Guardar</a>


            </div>


          </div>
        </div>

        <a href="#impresoratickets" class="modal-trigger anclaconfig">
          <img src="img/impresoradetickets.png" alt="" class="imgconfig">
          <span>Impresora de tickets</span>
        </a>


        <!--  impresora de tickets Modal Structure -->
        <div id="impresoratickets" class="modal">

          <h5 style="text-align: center;margin-top: 0;">
            Impresora de tickets
          </h5>

          <p style="margin-bottom: 10px;">Por favor, elige la impresora de tickets entre las impresoras instaladas en tu
            sistema:
          </p>

          <div class="continput">

            <div>
              <p class="left"> Impresora de Tickets:</p>

              <input id="" type="text" class="inputglobal">


            </div>


            <div>
              <p class="left"> Fuente de impresión normal:</p>

              <input id="" type="text" class="inputglobal">

            </div>


            <div>
              <p class="left">Columnas:</p>

              <input id="" type="text" class="inputglobal2">

            </div>

          </div>
          <div class="col s12">

            <input type="checkbox" class="filled-in left" checked="checked" />

            <span><strong class="black-text">Usar fuente "normal" para los
                totales</strong></span>


          </div>


          <div>

            <input type="checkbox" class="filled-in left" checked="checked" />
            <span><strong class="black-text">Poner todas las letras
                negritas</strong></span>
          </div>

          <div style="margin-top:10px;display: flex;justify-content: center;align-items: center;">
            <a id="" class="guardarcrear">
              Guardar</a>
          </div>
        </div>

        <div id="impresoradeetiquetas">
          <a href="#eliminarcliente" class="modal-trigger ancla">
            <img src="img/impresoradetickets.png" style="width:30px; margin-right:8px;">Impresora de etiquetas</a>
        </div>
      </div>
    </div>
  </div>


  <!--  Scripts-->
  <script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>

  <!-- Compiled and minified JavaScript -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/js/materialize.min.js"></script>
  
  <!-- Scripts -->
  <script src="{{ asset('js/index.js') }}" defer></script>




</body>

</html>